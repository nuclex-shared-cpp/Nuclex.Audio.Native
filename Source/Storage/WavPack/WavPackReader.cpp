...
namespace Nuclex { namespace Audio { namespace Storage { namespace WavPack {
  ...
  template<> void WavPackReader::DecodeInterleaved<std::uint8_t>(
    ...
    if(mode & MODE_FLOAT) {
      decodeInterleavedAndConvert<std::uint8_t, true>(target, frameCount);
    } else {
      decodeInterleavedAndConvert<std::uint8_t, false>(target, frameCount);
    }
  }
  ...
  template<> void WavPackReader::DecodeInterleaved<std::int16_t>(
    ...
    if(mode & MODE_FLOAT) {
      decodeInterleavedAndConvert<std::int16_t, true>(target, frameCount);
    } else {
      decodeInterleavedAndConvert<std::int16_t, false>(target, frameCount);
    }
  }
  ...
  template<> void WavPackReader::DecodeInterleaved<std::int32_t>(
    ...
    if(mode & MODE_FLOAT) {
      decodeInterleavedAndConvert<std::int32_t, true>(target, frameCount);
    } else {
      decodeInterleavedAndConvert<std::int32_t, false>(target, frameCount);
    }
  }
  ...
  template<> void WavPackReader::DecodeInterleaved<float>(
    ...
    if(mode & MODE_FLOAT) {
      ...
      decodeInterleavedAndConvert<float, false, true>(target, frameCount);
    } else {
      decodeInterleavedAndConvert<float, false, false>(target, frameCount);
    }
  }
  ...
  template<> void WavPackReader::DecodeInterleaved<double>(
    ...
    if(mode & MODE_FLOAT) {
      ...
      decodeInterleavedAndConvert<double, false, true>(target, frameCount);
    } else {
      decodeInterleavedAndConvert<double, false, false>(target, frameCount);
    }
  }
  ...
  template<typename TSample, bool FromFloat /* = false */, bool BitsPerSampleOver16 /* = false */>
  void WavPackReader::decodeInterleavedAndConvert(TSample *target, std::size_t frameCount) {
    ...
    while(0 < frameCount) {
      ...
      if constexpr(FromFloat) {
        ...
        // The target can only be a double or an integer type. If the target is float,
        // this method never gets invoked and the decode happens directly
        ...
        if constexpr(std::is_same<TSample, double>::value) {
          ...
          // Determine the 'limit' type to use. We don't use 'BitsPerSampleOver16' here
          // because this isn't about the bits per sample in the decoded WavPack data
          // but about the user-provided data type. For the same reason, the template
          // argument is not filled anyway on a call when the source type is float.
          typedef typename std::conditional<
            sizeof(TSample) < 17, float, double
          >::type LimitType;

          // Calculate the limit (the signal ceiling, meaning the integer value to which
          // the lowest and higher floating point value maps). This could be 32767.0 for
          // int16_t as the target type, for example.
